...
const USDT_ADDRESS = import.meta.env.VITE_USDT_MAINNET_ADDRESS || '0xc2132D05D31c914a87C6611C10748AEb04B58e8F';
const POLYGON_AMOY_RPC = import.meta.env.VITE_POLYGON_AMOY_RPC || 'https://polygon-amoy-bor-rpc.publicnode.com';
const UsdtABI = [{"inputs":[{"internalType":"address","name":"account","type":"address"}],"name":"balanceOf","outputs":[{"internalType":"uint256","name":"","type":"uint256"}],"stateMutability":"view","type":"function"}];
...
const Header = () => {
  ...
  useEffect(() => {
    if (user?.walletAddress) {
      ...
      usdt.balanceOf(user.walletAddress).then(bal => setWalletUsdt(Number(bal) / 1e6)).catch(() => {});
    }
