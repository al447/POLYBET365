...
const useBreakingAndHotTopics = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [featRes, volRes] = await Promise.all([
          marketsAPI.getMarkets({ featured: true, sort: 'volume', limit: 3, status: 'active' }),
          marketsAPI.getMarkets({ sort: 'volume', limit: 8, status: 'active' }),
        ]);

        // Breaking news: featured markets from your 6 categories
        const featuredMarkets = featRes.data?.markets || [];
        const breakingList = featuredMarkets.length >= 3
          ? featuredMarkets
          : (volRes.data?.markets || []).slice(0, 3);
        setBreaking(breakingList.slice(0, 3).map((m, i) => {
        ...
        const hotList = (volRes.data?.markets || [])
          .filter(m => !breakingSlugs.has(m.slug))
          .slice(0, 5);
        setHotTopics(hotList.map((m) => ({
...
const MarketChart = ({ data, lines, isCrypto, target }) => {
...
const FeaturedCarousel = () => {
  ...
  const { data: featuredData, isLoading } = useMarkets({ sort: 'volume', limit: 9, status: 'active' });
        ...
        <AnimatePresence mode="wait">
          ...
          >
            ...
            <div className="flex-1 relative">
              <MarketChart data={slide.chartData} lines={slide.chartLines} isCrypto={slide.isCrypto} target={slide.cryptoTarget} />
