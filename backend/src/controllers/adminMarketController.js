...
const create = async (req, res, next) => {
  try {
    const { title, description, categorySlug, image, endDate, closeDate, rules, sourceOfTruth, tags, featured, outcomes } = req.body;
    ...
    await audit.log({ admin: req.user._id, action: 'market_create', targetType: 'market', targetId: market._id, details: { title: market.title, categorySlug }, ipAddress: req.ip });
...
const resolve = async (req, res, next) => {
  try {
    ...
    await audit.log({ admin: req.user._id, action: 'market_resolve', targetType: 'market', targetId: market._id, details: { outcome, settledCount, title: market.title }, ipAddress: req.ip });

    invalidateCache('/api/markets');
    invalidateCache('/api/categories');

    res.json({ success: true, market, settledTrades: settledCount });
  } catch (e) { next(e); }
};
