...
const EXPLORERS = {
  ...
  bsc:            { mode: 'rpc', maxNativeBlocks: 6000 },  // ~5h @ 3s blocks (cold-start safety)
  base:           { mode: 'rpc', maxNativeBlocks: 9000 },  // ~5h @ 2s blocks (cold-start safety)
};
...
const getProvider = (chain) => {
  ...
  _providers[chain] = new ethers.JsonRpcProvider(url, undefined, { batchMaxCount: 10 });
...
const findUserBySender = async (sender) => {
  ...
  // Direct walletAddress match (case-insensitive regex)
 ...
 */
const creditDeposit = async ({ chain, token, txHash, sender, amount, blockNumber, note }) => {
  ...
  // Credit user balance
  await User.findByIdAndUpdate(user._id, { $inc: { balance: creditedAmountUsd } });
 ...
 */
// Parallel getBlock calls per batch. Capped at 10 because Base's public RPC
// rejects batches >10 with "maximum 10 calls in 1 batch" (-32014). BSC tolerates
// more but 10 is a safe universal default for public RPC providers.
const NATIVE_BATCH_SIZE = 10;
