...
const getOrCreateConfig = async () => {
  ...
  if (!config) {
    ...
    // Correct stale values if they were set before the program update
    const updates = {};
      ...
      await User.findByIdAndUpdate(
        referral.referrer,
        {
          $inc: {
            ...
            pendingReferralBalance: -referrerBonus,
          },
        },
        { session }
      );
      ...
      await User.findByIdAndUpdate(
        referral.referee,
        {
          $inc: {
            ...
            pendingReferralBalance: -refereeBonus,
          },
        },
        { session }
      );
...
const recordTradeForReferral = async ({ refereeUserId, tradeId, tradeAmount }) => {
  ...
  // Update qualifying volume if still pending
  if (referral.status === 'pending') {
    referral.qualifyingTradeVolume += tradeAmount;
    await referral.save();

    if (referral.qualifyingTradeVolume >= config.qualifyingTradeThreshold) {
